                ...
                switch ( menu() ) {
                        ...
                        case 2: {// Ввод сведений об оплате штрафа
                                ...
                                if ( !( p = search_insert( root, data, DEL, dir, parent ) ) )
                                        cout << "Svedenia ob a/m otsutstvuyut " << endl;
                        ...
                        case 3:// Справка
                                ...
                                if ( !(p = search_insert( root, input(INFO), INFO, dir, parent )))
                                        cout << "Svedenia otsutstvuyut " << endl;
                                else
                                        print_node( *p );
                                break;
                        case 4:	// Выход
                                fout.open( filename );
                                if ( !fout.is_open() ) {
                                        cout << "Oshibka otkritiya faila " << filename; return 1;
                                }
                                write_dbase( fout, root );
                                return 0;
                        case 5: print_dbase( root ); break;			// Отладка
                        default: cout << " Nado vvodit chislo ot 1 do 4" << endl; break;
                }
        }
}
...
Data input( Action action ) { // ---------------------------------Ввод нарушения
   ...
   do { cout << "Vvedite datu narusheniya v formate DD.MM.GG:" << endl; cin >> buf;
   ...
   do { cout << "Vvedite time narusheniya v formate HH:MM :" << endl; cin >> buf;
   ...
   if ( action == DEL ) return data;
       cout << "Vvedite type narusheniya " << endl; cin.getline( data.type, l_type );
   do { cout << "Vvedite razmer shtrafa:" << endl; cin >> buf;
...
int menu() {// -------------------------------------------Вывод меню
   char buf[10];
   int option;
   do { cout << "==================================" << endl; cout << "1 - Vvod svedenii o narushenii" << endl; cout << "2 - Vvod svedenii ob oplate shtrafa" << endl; cout << "3 - Spravka" << endl;
      cout << "4 - Vihod" << endl;
...
void print_node( const Node& node ) { // --------Вывод на экран сведений об а/м cout << "Номер а/м: " << node.number << endl;
   ...
   while ( pf ) {
      cout << "Vid narusheniia: " << pf->type<< endl; cout << "Data and time: " <<pf->time;
      cout << "\nRazmer shtrafa: " << pf->price<< endl; summa +=pf->price;
      pf = pf->next;
   }
   cout << "Obshaya summa shtrafov: " << summa << endl;
}
...
Node * read_dbase ( char* filename ) { //-------------------Чтение базы из файла
     ...
     if ( !f ) { cout << "Net faila " << filename << endl; return NULL; }
     f.getline( data.number, l_number );// Номер а/м
     if( f.eof( ) ) { cout << "Pustoi fail ( 0 байт )" << endl; return NULL; }
...
int remove_fine( Node* p, const Data& data ) {
   ...
   if ( !found ) { cout << "Svedenia o narushenii otsutstvuyt." << endl; return 1; }
   if ( pf == p->beg)
